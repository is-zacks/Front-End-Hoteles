// app/gestion/habitaciones/DashboardMantenimiento.js
...
const opcionesMantenimiento = [
  ...
  { estado: 'fuera de servicio', label: 'Fuera de Servicio', icon: 'alert-circle-outline' },
];
...
export default function DashboardMantenimiento() {
    ...
    setHabitaciones((prev) =>
      prev.map((hab) =>
        hab.id === id
          ? {
              ...hab,
              [tipo === 'mantenimiento' ? 'estado_mantenimiento' : 'estado_limpieza']: nuevoEstado,
            }
          : hab
      )
    );
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        className={`flex-1 min-h-[140px] rounded-2xl justify-center items-center m-2 p-4 ${fondoMantenimiento} shadow-md`}
        onPress={() => {
  ...
  return (
    <BaseScreen>
      <View className="p-4">
        <Text className="text-3xl font-bold text-[#2c2c66] mb-4">Dashboard de Mantenimiento</Text>
        <FlatList
          data={habitaciones}
        ...
        <CambioEstadoModal
          ...
          opciones={modalTipo === 'mantenimiento' ? opcionesMantenimiento : []}
